...
// 硬编码默认参数
const SAKURA_DEFAULTS: Omit<SakuraInternalConfig, "enable"> = {
    sakuraNum: 21,
    limitTimes: -1,
...
function buildFullConfig(config: SakuraConfig): SakuraInternalConfig {
    return { ...SAKURA_DEFAULTS, enable: config.enable };
}
...
function getRandom(option: string, config: SakuraInternalConfig) {
...
export class SakuraManager {
    private config: SakuraInternalConfig;
    ...
    toggle(): void {
        if (this.isRunning) {
            ...
            void this.init().catch((error: unknown) => {
                console.warn("[sakura-manager] init failed in toggle", error);
            });
        }
    }
    ...
    updateConfig(newConfig: SakuraConfig): void {
        ...
        this.config = buildFullConfig(newConfig);
        if (wasRunning && newConfig.enable) {
            void this.init().catch((error: unknown) => {
                console.warn(
                    "[sakura-manager] init failed after config update",
                    error,
                );
            });
        }
...
export function initSakura(config: SakuraConfig): void {
    if (globalSakuraManager) {
        ...
        if (config.enable) {
            void globalSakuraManager.init().catch((error: unknown) => {
                console.warn(
                    "[sakura-manager] init failed in initSakura",
                    error,
                );
            });
        }
